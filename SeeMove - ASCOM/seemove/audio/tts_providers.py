...
class ElevenLabsTTSProvider(TTSProvider):
    ...
    def synthesize(self, text: str, rate: int, volume: float,
        ...
        # output_format é query param na API da ElevenLabs, não campo do
        # corpo JSON — estava dentro de `payload` e por isso era ignorado
        # silenciosamente, fazendo a API cair no formato padrão (MP3), que
        # nem sempre é decodificável pelo soundfile/libsndfile instalado.
        params = {"output_format": f"pcm_{self._PCM_SAMPLE_RATE}"}
        try:
            resp = self._requests.post(
                url, json=payload, headers=headers, params=params, timeout=15
            )
            resp.raise_for_status()
            # pcm_16000 devolve PCM cru (sem cabeçalho) — soundfile.read()
            # (usado por TTSEngine._play_audio) exige um container
            # autodescritivo pra detectar sample rate/canais; sem isso a
            # síntese "funcionava" mas nunca tocava nada. Empacota o PCM
            # num WAV mínimo aqui mesmo, já que sabemos exatamente a
            # taxa/formato que pedimos.
    ...
    def _pcm_to_wav(self, pcm_bytes: bytes) -> bytes:
        ...
        with wave.open(buf, "wb") as w:
            ...
            w.setframerate(self._PCM_SAMPLE_RATE)
...
# Cache LRU em memória para frases repetidas (confirmações, mensagens
# fixas de início/fim) — evita custo/latência de rechamar a API de nuvem.
# OrderedDict em vez de dict simples: um dict simples descartava por ordem
# de INSERÇÃO (o mais antigo cadastrado, não o menos usado), então uma
# frase repetida com frequência (ex. confirmação) podia ser despejada em
# favor de outra falada só uma vez há muito tempo — voltava a chamar a
# nuvem exatamente nas frases mais comuns, o oposto do que um cache deveria
# fazer.
_SYNTH_CACHE: "OrderedDict[tuple, Optional[bytes]]" = OrderedDict()
...
def synthesize_cached(provider: TTSProvider, provider_name: str, text: str,
    ...
    if key in _SYNTH_CACHE:
        _SYNTH_CACHE.move_to_end(key)  # marca como usado agora (LRU)
        return _SYNTH_CACHE[key]

    audio = provider.synthesize(text, rate, volume, voice_id)
    if audio is not None:
        if len(_SYNTH_CACHE) >= _SYNTH_CACHE_MAX:
            _SYNTH_CACHE.popitem(last=False)  # descarta o menos usado recentemente
