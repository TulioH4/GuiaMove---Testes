...
Eventos Socket.IO recebidos:
  audio_done      — {id}: o browser terminou (ou teve cortada) a fala `id`
  request_image   — pede o quadro de imagem atual
"""
...
# Sem isso, com debug=False (produção), o Jinja cacheia dashboard.html na
# primeira renderização e ignora edições no arquivo até reiniciar o
# processo — mordeu durante o desenvolvimento ao vivo desta sessão. Custo
# é só um stat() do arquivo por request, irrelevante nesse volume de uso.
...
def inject_refs(session, tracker, reporter, settings, tts, registry):
    ...
    with _lock:
        ...
        # Chave do REGISTRY (squat/stand/balance), que é o que os botões do
        # dashboard usam — derivar do nome da classe dava "staticposture"/
        # "unipodialbalance" e a aba inicial não batia com o exercício real.
        _state["exercise"] = next(
            (k for k, cls in getattr(registry, "_REGISTRY", {}).items()
             if isinstance(session.exercise, cls)),
            "squat",
        )
...
def push_frame(frame, result, summary):
    ...
    # Lê o que precisa do lock da SESSÃO antes de pegar o lock do servidor.
    # A thread de análise faz o inverso (segura o lock da sessão e, ao falar,
    # chama push_tts_log, que pega o do servidor): pegar os dois em ordem
    # oposta aqui era um deadlock (ABBA) esperando acontecer — congelava
    # análise, boneco e áudio de uma vez, sem nenhuma mensagem.
    session_state = _session_state_for_ui()
    ex = None
    ...
    with _lock:
        _state["session_state"]  = session_state
        ...
        _state["image_aspect"]   = frame.image_aspect
        ...
        # Lê sob _session._lock: essa thread (broadcast) é diferente da que
        # troca `_session.exercise` (set_exercise()/start_exercise(), sob o
        # mesmo lock) — sem isso é uma leitura não sincronizada de um
        # atributo que outra thread pode reatribuir a qualquer momento.
        if ex is not None:
...
def tracker_connect():
    ...
    _tracker.use_depth    = use_depth and getattr(_tracker, "depth_capable", False)
...
def kinect_tilt():
    """
    Move o motor de inclinação físico do Kinect v1 (-27 a 27 graus). Só
    funciona no modo --kinect-sdk — em webcam comum não há motor. Aceita
    {"angle": N} (absoluto) ou {"delta": N} (relativo ao último ângulo
    pedido, pra botões tipo "▲"/"▼" no dashboard).
    """
    if _tracker is None:
        return jsonify({"error": "sessão não inicializada"}), 500
    data = request.get_json(silent=True) or {}
...
@app.route("/api/kinect/tilt")
def kinect_tilt_status():
    if _tracker is None:
        return jsonify({"error": "sessão não inicializada"}), 500
    status = _tracker.get_tilt_status()
    if status is None:
...
def report_csv():
    ...
    w.writeheader()
    with _reporter._records_lock:
        records = list(_reporter._records)
    for r in records:
        w.writerow({
...
def on_connect():
    ...
    # Só pro cliente que acabou de conectar (socketio.emit fazia broadcast pra
    # TODAS as abas abertas a cada nova conexão) e SEM a pose antiga: ela era
    # reaplicada no boneco 3D como se fosse uma leitura nova.
    snap["raw_landmarks"] = []
    emit("frame", snap)
    emit("tracker_status", {
...
def on_audio_done(data):
    """O navegador terminou de falar a mensagem `id` (ver AudioClient.speak) —
    dá ao servidor o fim REAL da fala em vez da estimativa por nº de palavras
    (briefing, calibração e "não cortar fala em andamento" dependem disso)."""
    if _session is None or not isinstance(data, dict):
        return
    try:
        _session.audio.speech_finished(int(data.get("id", -1)))
...
def start(port: int = 5000, open_browser: bool = True):
    # Outra instância já usando a porta: o servidor novo morria em silêncio
    # (thread daemon) e o navegador abria o dashboard da instância ANTIGA,
