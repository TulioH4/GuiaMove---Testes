...
class SquatExercise(Exercise):
    ...
    """
    name          = "Agachamento"
    # Meta de repetições da série — Session lê este atributo (getattr) pra
    # dizer quantas faltam depois de cada repetição; exercícios sem esse
    # atributo simplesmente não têm meta.
    target_reps   = 5
    ...
    # Profundidade mínima aceitável. Valor original (115°) era calibrado
    # pra um agachamento de academia bem profundo — em teste real com
    # webcam (distância/altura de câmera típicas, público desta
    # acessibilidade), o ângulo do joelho raramente passou de ~150-153°
    # mesmo num agachamento feito "corretamente", então a repetição nunca
    # confirmava profundidade suficiente e sempre abortava como "raso".
    # 155° dá uma margem confortável abaixo do que foi medido de verdade.
    # Ainda é um valor fixo, não adaptado por pessoa/câmera — se continuar
    # não contando ou contar rápido demais pra outra pessoa/setup, é só
    # ajustar este número (ou pedir pra eu tornar isso configurável).
    BOTTOM_ANGLE        = 155.0
    ...
    MIN_REP_DURATION_S = 0.9   # mais rápido que isso: descida/subida bruscas
    ...
    def __init__(self):
        ...
        # Ciclos que começaram a descer mas voltaram a ficar em pé sem chegar
        # à profundidade mínima — Session usa pra avisar POR QUE não contou.
        self.shallow_reps  = 0
        # Ciclos completos (fundo atingido e voltou) que NÃO contam porque
        # teve um erro GRAVE de postura (Severity.ERROR) durante o movimento.
        self.posture_rejected_reps = 0
        # Pior problema de postura visto durante o ciclo que acabou de
        # fechar (contado, rejeitado ou raso) — mensagem pronta pra falar, ou
        # None se o movimento foi limpo. Session fala isso no fim da repetição.
        self.last_rep_issue: Optional[str] = None
        self._cycle_worst: Optional[FeedbackResult] = None
        ...
        # Profundidade real atingida na última repetição (o menor ângulo
        # de joelho visto durante o ciclo) — útil pra calibrar/ajustar
        # BOTTOM_ANGLE com base no que a pessoa de fato consegue alcançar,
        # em vez de só adivinhar. `_current_min_angle` é o acumulador da
        # tentativa em andamento (resetado a cada nova descida);
        # `last_rep_min_angle` é o valor público, congelado no momento em
        # que a repetição fecha (conte ou não pela cadência).
        self._current_min_angle = 180.0
        self.last_rep_min_angle: float = 180.0
    ...
    def analyze(self, frame: SkeletonFrame) -> FeedbackResult:
        ...
        shallow_before = self.shallow_reps
        posture_rej_before = self.posture_rejected_reps
        repetition_completed = self._update_movement_state(angle)
        if repetition_completed or self.shallow_reps != shallow_before:
        ...
        detail = (
            ...
            f"cadencia={self.last_rep_cadence};menor_angulo={self.last_rep_min_angle:.1f}"
        )

        if repetition_completed:
            if self.posture_rejected_reps != posture_rej_before:
                msg = "Não contou — erro de postura. Tente de novo."
            else:
        ...
        if posture_result.severity != Severity.OK and self._movement_state != self.STATE_STANDING:
            # só guarda o que aconteceu DENTRO de um ciclo de repetição
            self._cycle_worst = (posture_result if self._cycle_worst is None
                                 else worst_feedback([self._cycle_worst, posture_result]))
    ...
    def _check_valgus(self, frame: SkeletonFrame):
        m = frame.metrics
        # knee_valgus_* é ASSINADO: positivo = joelho pra DENTRO (valgo),
        # negativo = pra FORA da linha quadril-tornozelo. Só o positivo é o
        # erro que esta checagem descreve ("cedendo pra dentro"). Com abs(), o
        # joelho aberto pra fora — que é o correto no agachamento e chega a
        # -5..-6% na descida — disparava "joelho esquerdo para dentro" (o
        # lado com o maior desvio em módulo ganhava o desempate).
        vl, vr = max(0.0, m.knee_valgus_l), max(0.0, m.knee_valgus_r)
        worst = max(vl, vr)
    ...
    def _average_knee_angle(self, frame: SkeletonFrame) -> float:
        ...
        Checa a VISIBILIDADE real do quadril/joelho/tornozelo em
        frame.points, não só se frame.metrics.knee_angle_* "parece" um
        valor razoável — compute_metrics() (core/skeleton.py) deixa esse
        campo no valor padrão (180.0, exatamente "em pé reto") quando não
        consegue calcular por falta de algum landmark visível, e um
        ângulo antigo simplesmente comparar contra "0 < x <= 180" não
        distingue "genuinamente em pé" de "não deu pra calcular, ficou no
        padrão". Isso trava a máquina de fases em EM_PÉ pra sempre se o
        tornozelo sair de quadro durante a descida (comum: o enquadramento
        que passou na calibração parado pode não sobrar tornozelo visível
        quando a pessoa se abaixa e a câmera está um pouco alta/próxima)."""
        pts = frame.points
        valid = []

        lh, lk, la = pts.get(L_HIP), pts.get(L_KNEE), pts.get(L_ANKLE)
        if lh and lk and la and lh.visible and lk.visible and la.visible:
            valid.append(frame.metrics.knee_angle_l)

        rh, rk, ra = pts.get(R_HIP), pts.get(R_KNEE), pts.get(R_ANKLE)
        if rh and rk and ra and rh.visible and rk.visible and ra.visible:
            valid.append(frame.metrics.knee_angle_r)
    ...
    def _update_movement_state(self, angle: float) -> bool:
        ...
        # Acompanha o ponto mais fundo da tentativa em andamento — não
        # interfere em nenhuma transição de fase, é só telemetria.
        if self._movement_state in (self.STATE_DESCENDING, self.STATE_BOTTOM):
            if angle < self._current_min_angle:
        ...
        if self._movement_state == self.STATE_STANDING:
            if angle < self.DESCENT_ENTER_ANGLE:
                ...
                # STATE_CONFIRM_FRAMES da linha abaixo) — senão o ponto
                # mais fundo real, que costuma acontecer bem nesses
                # primeiros frames, ficava de fora por uns décimos de
                # grau assim que a fase virava oficialmente "descendo".
                if angle < self._current_min_angle:
                    self._current_min_angle = angle
        ...
        if self._movement_state == self.STATE_DESCENDING:
            if angle <= self.BOTTOM_ANGLE:
                # Chegar à profundidade é um evento PONTUAL — diferente de
                # STANDING↔DESCENDING (onde exigir vários frames evita
                # ruído de alguém só balançando o peso parado), aqui exigir
                # STATE_CONFIRM_FRAMES consecutivos ABAIXO do limiar
                # praticamente nunca confirmava: um agachamento real não
                # fica parado no fundo, e a confirmação de DESCENDING já
                # tinha acabado de consumir vários frames bem no meio da
                # descida — sobrava só 1-2 frames de fundo de verdade
                # antes da pessoa já estar subindo de novo, nunca o
                # bastante pra bater os 4 frames exigidos. Ângulo já vem
                # suavizado (EMA em _smooth_angle), então confirmar na
                # hora aqui é seguro o suficiente.
                self._movement_state = self.STATE_BOTTOM
                self._candidate_state, self._candidate_frames = None, 0
            elif angle > self.STAND_EXIT_ANGLE:
                # Voltou a ficar em pé sem chegar ao fundo — agachamento
                # raso, aborta sem contar repetição (e avisa, via shallow_reps).
                self.shallow_reps += 1
                self._movement_state = self.STATE_STANDING
                self._candidate_state, self._candidate_frames = None, 0
                self._movement_started_at = None
        ...
        if self._movement_state == self.STATE_BOTTOM:
            if angle > self.BOTTOM_ANGLE + 5.0:
                # Mesmo raciocínio: sair do fundo também é pontual, não
                # precisa de confirmação por vários frames.
                self._movement_state = self.STATE_ASCENDING
                self._candidate_state, self._candidate_frames = None, 0
            return False
        ...
        if self._movement_state == self.STATE_ASCENDING:
            if angle >= self.STAND_EXIT_ANGLE and self._request_state(self.STATE_STANDING):
                ...
                if self._cycle_worst is not None and self._cycle_worst.severity == Severity.ERROR:
                    # Erro grave de postura no ciclo: não conta, independente
                    # do ritmo (é o mais importante de corrigir).
                    self.posture_rejected_reps += 1
                elif self.last_rep_cadence == "boa":
                    self.repetitions += 1
                else:
