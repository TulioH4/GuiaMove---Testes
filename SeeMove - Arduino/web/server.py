...
Rotas:
  ...
  POST /api/hardware/connect  Inicia conexão com a plataforma real
...
@app.route("/figure3d")
def figure3d():
    return render_template("figure3d.html")
...
def hw_connect():
    """Inicia conexão com a plataforma real em thread separada."""
    if _board_ref is None:
        return jsonify({"error": "sessão não inicializada"}), 500

    def _do_connect():
        push_hw_status("Conectando ao Arduino pela porta serial...", False)
        ok = _board_ref.connect()
        if ok:
            push_hw_status("Plataforma Arduino conectada!", True)
            socketio.emit("hw_connected", {})
        else:
            push_hw_status("Não foi possível conectar. Verifique porta serial, cabo USB e baud rate.", False)
