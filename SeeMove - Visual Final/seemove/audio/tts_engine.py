...
class TTSEngine:
        ...
        if priority:
            while not self._queue.empty():
                try:
                    ...
                    break
            # Prioridade também corta a fala EM ANDAMENTO (no dashboard, o
            # navegador já faz isso com cancel=True): antes só a fila era
            # descartada e o briefing esperava a frase anterior acabar.
            self._cut_current()
    ...
    def _cut_current(self):
        """Interrompe a fala em andamento (subprocess pyttsx3 ou áudio de
        nuvem), sem mexer na fila."""
        with self._proc_lock:
