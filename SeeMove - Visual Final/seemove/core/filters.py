    ...
    def _get(self, idx: int) -> Dict[str, OneEuroFilter]:
        ...
        if f is None:
            f = {
                ...
                # Z (profundidade monocular) é o eixo mais ruidoso — filtro
                # bem mais pesado que X/Y: corta o tremor do joelho/tornozelo
                # à custa de um pouco de atraso só nesse eixo.
                "z": OneEuroFilter(self.min_cutoff * self.Z_CUTOFF_FACTOR, self.beta * self.Z_BETA_FACTOR),
            }
            self._filters[idx] = f
        return f
