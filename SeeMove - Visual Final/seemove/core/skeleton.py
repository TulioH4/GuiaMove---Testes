...
# Brilho médio (0-255) abaixo do qual a imagem é escura demais pra detectar
# uma pessoa. Sala normal iluminada fica em ~60-150; câmera tapada/sala
# apagada em ~0-5 (medido no Kinect: exatamente 0 no escuro).
DARK_BRIGHTNESS = 20.0
DARK_MESSAGE = "A imagem da câmera está escura demais. Acenda a luz do ambiente."
...
class Point3D:
    ...
    z: float
    visibility: float
    # True quando o Z veio de profundidade REAL do sensor (Kinect IR), não da
    # estimativa monocular do MediaPipe — o boneco 3D confia bem mais nele.
    real_z: bool = False
...
class SkeletonFrame:
    ...
    image_aspect: float = 4 / 3       # largura/altura da imagem (corrige a escala de x no boneco 3D)
    # Brilho médio da imagem (0-255). Sala escura = o MediaPipe não acha
    # ninguém, e quem não enxerga não tem como saber que a luz está apagada.
...
def _tilt_deg(left: Point3D, right: Point3D) -> float:
    ...
    Positivo = lado DIREITO da pessoa mais alto (labels anatômicos corretos:
    o tracker detecta na imagem original e só depois espelha o x).
    """
    if not (left.visible and right.visible):
        return 0.0
    dy = left.y - right.y   # y aumenta para baixo na imagem; positivo = direita MAIS ALTA
