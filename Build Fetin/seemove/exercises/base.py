...
class Exercise(ABC):
    ...
    # Confiança mínima (média de visibilidade, em %) pra analisar o quadro.
    MIN_CONFIDENCE = 40.0

    def detection_lost(self, frame: SkeletonFrame) -> bool:
        """True quando o quadro não tem uma pessoa utilizável (sem esqueleto
        ou confiança abaixo do mínimo) — mesmo critério de _require_detection,
        exposto pra Session tolerar quedas curtas antes de dar a pessoa como
        perdida."""
        return (not frame.detected) or frame.metrics.confidence < self.MIN_CONFIDENCE
        ...
        if not frame.detected:
            if frame.brightness < DARK_BRIGHTNESS:
                return FeedbackResult(
                    DARK_MESSAGE, True, Severity.WARN,
                    f"Imagem escura (brilho {frame.brightness:.0f}/255)."
                )
            return FeedbackResult(
