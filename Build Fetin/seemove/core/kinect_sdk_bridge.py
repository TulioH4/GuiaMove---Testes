...
class KinectSDKBridge:
    ...
    @staticmethod
    def _read_stable(mm, nbytes: int, last_counter: int):
        """Lê o quadro da memória compartilhada e confere o contador DEPOIS: a
        ponte escreve o quadro no mesmo lugar e só então incrementa o contador,
        então se ele mudou durante a nossa leitura, o quadro pode ter vindo
        "rasgado" (metade de cada quadro) — lê de novo (o quadro novo já está
        lá). Devolve (bytes, contador) ou (None, contador) se não há quadro novo."""
        raw, counter = None, 0
        for _ in range(3):
            mm.seek(0)
            counter = struct.unpack("<Q", mm.read(8))[0]
            if counter == 0 or counter == last_counter:
                return None, counter
            mm.seek(HEADER_BYTES)
            raw = mm.read(nbytes)
            mm.seek(0)
            if struct.unpack("<Q", mm.read(8))[0] == counter:
                break
        return raw, counter
    ...
    def read(self) -> Optional[np.ndarray]:
        ...
        raw, counter = self._read_stable(self._mm_color, COLOR_FRAME_BYTES, self._last_color_counter)
        if raw is None:
            return None
    ...
    def read_depth(self) -> Optional[np.ndarray]:
        ...
        raw, counter = self._read_stable(self._mm_depth, DEPTH_FRAME_BYTES, self._last_depth_counter)
        if raw is None:
            return None
