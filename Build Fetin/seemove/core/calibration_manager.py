...
from core.skeleton import (
    SkeletonFrame, Point3D, MIN_VIS, DARK_BRIGHTNESS, DARK_MESSAGE,
    NOSE, L_EAR, R_EAR,
...
def _check_dark(frame: SkeletonFrame) -> Optional[FramingResult]:
    # Antes de "nenhuma pessoa detectada": no escuro o MediaPipe nunca acha
    # ninguém e a instrução certa é acender a luz, não "posicione-se".
    if frame.brightness < DARK_BRIGHTNESS:
        return FramingResult(False, FramingIssue.TOO_DARK, DARK_MESSAGE)
    return None
...
_CHECKS: List[_FramingCheck] = [
    _FramingCheck("dark",      _check_dark),
...
class CalibrationManager:
    ...
    # O MediaPipe pode perder/recuperar um landmark de um frame pro outro
    # (ruído normal, principalmente durante a própria calibração — a pessoa
    # está se ajustando). Sem essa espera, um problema diferente relatado
    # por só 1 frame de ruído já vencia a checagem seguinte e "roubava" a
    # próxima fala do cooldown, soando como se uma correção sobrescrevesse
    # a outra sem parar (ex.: "mãos não aparecem" -> "pessoa não
    # detectada" -> "mãos..." num ciclo, sem o problema real ter mudado).
    # Só fala um problema depois dele se manter o MESMO por essa janela.
    ISSUE_STABLE_S = 1.0
    ...
    def feed(self, frame: SkeletonFrame) -> FramingResult:
        ...
        if result.ok:
            ...
            if self._ok_since is None:
                ...
                # ela sabe se um exercício já estava pendente — nesse caso a
                # frase se junta ao briefing em vez de ser cortada por ele.
                self.on_success()
            return result

        self._ok_since = None

        if result.issue != self._issue_type:
            self._issue_type = result.issue
            self._issue_since = now
        issue_stable = now - self._issue_since >= self.ISSUE_STABLE_S

        # Nunca corta uma fala em andamento (ex.: a frase de abertura "Vamos
        # calibrar o enquadramento...") só porque o cooldown já passou —
        # sem esse cheque, o "Nenhuma pessoa detectada" periódico cortava a
        # introdução no meio quase toda vez (o cooldown de 4.5s costuma ser
